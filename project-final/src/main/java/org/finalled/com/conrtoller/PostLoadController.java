...
public class PostLoadController {
	...
	/*
	 * @GetMapping("/fileDownLoad/{fileName:.+}") public ResponseEntity<Resource>
	 * downloadFile(@PathVariable String fileName,HttpServletRequest request){
	 * 
	 * Resource resource = fileStorageService.loadFileAsResource(fileName);
	 * 
	 * String contentType = null; try { contentType =
	 * request.getServletContext().getMimeType(resource.getFile().getAbsolutePath())
	 * ; }catch(IOException ex) {
	 * logger.info("Could not determine file typeeeeeee"); } if(contentType == null)
	 ...
	 * public String createPostLoad(@RequestParam("file") MultipartFile file,
	 * PostLoad postload) { String fileName = fileStorageService.storeFile(file);
	 * String fileDownloadUri =
	 * ServletUriComponentsBuilder.fromCurrentContextPath().path("/fileDownLoad/")
	 ...
	 * 
	 * postLoadService.createPostLoad(postload); return "Successfully Created"; }
	 * 
	 * 
	 * 
	 */

	@RequestMapping(value = "/post_loads.php", method = RequestMethod.POST, consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = {
			MediaType.APPLICATION_ATOM_XML_VALUE, MediaType.APPLICATION_JSON_VALUE })

	@ResponseBody
	public String createPostLoad(PostLoad postload) {
